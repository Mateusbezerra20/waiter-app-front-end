...
export function Menu() {
  ...
  const tableReadyProducts = useMemo(() => {
    const prod = products.map((product) => {
      const productsCategory = categories.find((category) => category._id === product.category);
      return {
        ...product,
  ...
  return (
    <>
      <Header
        ...
        pageIcon="menu"
      />
      {isModalOpen && <NewProductModal handleSubmit={handleSaveNewProdut} handleClose={() => setIsModalOpen(false)} />}
      <Sections activeSection={activeSection} changeSection={changeActiveSection} />
      {
        activeSection === 'products'
          ? <ProductsTable data={tableReadyProducts} onNewProduct={() => setIsModalOpen(true)} />
